...
int main(int argc, char** argv) {
   ...
   if (options.getBoolean("list")) {
      ...
      cerr << "Usage: " << options.getCommand() << " [-t #|-l] midifile(s)" << endl;
   }
...
void setTempo(Options& options) {
   ...
   // 36000 / 60 = 600 rows.
   int tpq = int(6.0 * options.getDouble("tempo") + 0.5);
   if (tpq < 1)      { tpq = 1; }
...
void displayTempo(Options& options) {
   ...
   if (options.getArgCount() == 0) {
      midifile.read(cin);
      cout << midifile.getTicksPerQuarterNote() / 6.0 << endl;
   } else {
      for (int i=0; i<options.getArgCount(); i++) {
         midifile.read(options.getArg(i+1));
